...
env.Append(
    ...
    MICROAPP_MAKE_SCRIPT = join(platform.get_package_dir("tool-cs-header-maker"), 'microapp_make.py'),
)
...
firmware_elf_tmp = env.build_elf_tmp(source="$MAIN", target="$MAIN_ELF_TMP")
...
firmware_elf = env.build_elf(target='$MAIN_ELF', source='$MAIN')
...
upload_protocol = env.substr('$UPLOAD_PROTOCOL')
